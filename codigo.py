...
# Classe Proxy para Customer
class CustomerProxy:
    def __init__(self, customer):
        self._customer = customer

    def view_personal_info(self):
        # Lógica de verificação de autorização antes de retornar as informações pessoais
        # pode adicionar lógica para verificar se o usuário tem permissão para visualizar informações pessoais
        # Por simplicidade, implemtei acesso direto aqui, mas  pode adicionar verificações adicionais conforme necessário
        return self._customer.view_personal_info()

    def add_policy(self, policy):
        # Lógica de verificação de autorização antes de retornar as informações pessoais
        # pode adicionar lógica para verificar se o usuário tem permissão para visualizar informações pessoais
        # Por simplicidade, implemtei acesso direto aqui, mas  pode adicionar verificações adicionais conforme necessário
        self._customer.add_policy(policy)

    # Outros métodos podem ser implementados de maneira semelhante, se necessário
...
class PolicyProxy:
    ...
    def view_policy_details(self):
        ...
        # pode adicionar lógica para verificar se o usuário tem permissão para visualizar informações pessoais
        # Por simplicidade, implemtei acesso direto aqui, mas  pode adicionar verificações adicionais conforme necessário
        return self._policy.view_policy_details()

    def process_payment(self, payment_amount):
        # Lógica de verificação de autorização antes de retornar as informações pessoais
        # pode adicionar lógica para verificar se o usuário tem permissão para visualizar informações pessoais
        # Por simplicidade, implemtei acesso direto aqui, mas  pode adicionar verificações adicionais conforme necessário
        return self._policy.process_payment(payment_amount)
    ...
    while True:
        ...
        if choice == '5':
            ...
            if 0 <= customer_index < len(customers) and 0 <= policy_index < len(policies):
                # Use CustomerProxy ao invés de Customer diretamente
                customer_proxy = CustomerProxy(customers[customer_index])
                customer_proxy.add_policy(policies[policy_index])
